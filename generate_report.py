...
cnx = mysql.connector.connect(host=credentials['host'], user=credentials['user'],
                              database=credentials['database'], password=credentials['password'])
cursor = cnx.cursor()
...
def get_dept_name(dept_no):
    cursor.execute(
        'select dept_name from departments where dept_no = %s;', (dept_no,))
    try:
...
def generate_employee_salaries_for_date_range(daterange):
    ...
    cursor.execute(query, (daterange.start, daterange.start,
