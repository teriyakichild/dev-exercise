...
def test_generate_quarter_date_ranges():
    ...
    assert range1.start == datetime.datetime.strptime(
        range1.start_string, '%Y-%m-%d').date()
...
def test_generate_quarter_date_ranges_with_mocked_start_end_dates(mock_newest, mock_oldest):
    mock_oldest.return_value = datetime.datetime.strptime(
        '1990-01-01', '%Y-%m-%d').date()
    mock_newest.return_value = datetime.datetime.strptime(
        '2000-09-30', '%Y-%m-%d').date()
    ranges = generate_quarter_date_ranges()
    ...
    mock_oldest.return_value = datetime.datetime.strptime(
        '1990-01-01', '%Y-%m-%d').date()
    mock_newest.return_value = datetime.datetime.strptime(
        '2000-10-01', '%Y-%m-%d').date()
    ranges = generate_quarter_date_ranges()
    assert len(ranges) == 44

    mock_oldest.return_value = datetime.datetime.strptime(
        '1989-12-30', '%Y-%m-%d').date()
    mock_newest.return_value = datetime.datetime.strptime(
        '2000-10-01', '%Y-%m-%d').date()
    ranges = generate_quarter_date_ranges()
